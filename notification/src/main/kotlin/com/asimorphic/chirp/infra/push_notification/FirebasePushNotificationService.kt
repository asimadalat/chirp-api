...
class FirebasePushNotificationService(
    ...
    fun initialise() {
        try {
            ...
            logger.info("Successfully initialised Firebase Admin SDK")
        } catch (ex: Exception) {
            logger.error("Failed to initialise Firebase Admin SDK", ex)
            throw ex
        }
    }
    ...
    fun isValidToken(token: String): Boolean {
        ...
        return try {
            ...
            logger.warn("Failed to validate Firebase token", ex)
            false
        }
    }
            ...
            Message
                ...
                .apply {
                    ...
                    when (recipient.platform) {
                        Platform.ANDROID -> setAndroidConfig(
                            AndroidConfig
                                .builder()
                                .setPriority(AndroidConfig.Priority.HIGH)
                                .setCollapseKey(notification.chatId.toString())
                                .setRestrictedPackageName("com.asimorphic.chirp")
                                .build()
                        )

                        Platform.IOS -> setApnsConfig(
                            ApnsConfig
                                .builder()
                                .setAps(
                                    Aps.builder()
    ...
    private fun BatchResponse.toSendResult(
        ...
        responses.forEachIndexed { index, sendResponse ->
            ...
            if (sendResponse.isSuccessful) {
                succeeded.add(allDeviceToken[index])
            } else {
                val errorCode = sendResponse.exception?.messagingErrorCode
                logger.warn("Failed to send notification to device with token ${deviceToken.token}: $errorCode")
        ...
        logger.debug("Push notifications sent. Succeeded: ${succeeded.size}, Temporary Failures: " +
                "${temporaryFailures.size}, Permanent Failures: ${permanentFailures.size}")
